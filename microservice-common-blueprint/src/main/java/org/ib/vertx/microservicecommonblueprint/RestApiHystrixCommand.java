        ...
        client.get(port, host, requestURI, response -> {
            ...
            if (response.statusCode() != 200) {
                ...
                response.endHandler( handler -> {
                    ...
                    String responseString = String.format("[%s][ResponseId-%d]-%s", Thread.currentThread().getName(), Thread.currentThread().getName(), ThreadLocalRandom.current().nextInt(Integer.MAX_VALUE), totalBuffer.toString());
